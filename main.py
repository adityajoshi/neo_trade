...
def login_client(totp, cred_details):
    client = NeoAPI(
        environment="prod",
        access_token=None,
        neo_fin_key=cred_details["neo_fin_key"] or None,
        consumer_key=cred_details["consumer_key"],
    )
    if client:
        client.totp_login(
            mobile_number=cred_details["mobno"], ucc=cred_details["ucc"], totp=totp
        )
...
def get_holdings(client):
    try:
        ...
        if "data" in response and response["data"]:
            ...
            print(f"This is the response from API: {response}.")
    except Exception as e:
        print(f"Error fetching holdings: {e}")


def book_trade(client, trade_details):
    try:
        ...
        print(f"Order response for {stock_id}: {response}")
        ...
        if isinstance(response, dict) and response.get("stat") == "Ok":
            ...
            error = response.get("error", response) if response else "No response"
            print(f"Order rejected for {stock_id}: {error}")
            return False
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Place orders from CSV using NeoAPI."
        )
        parser.add_argument(
            "--csv", default="trades.csv", help="Path to the CSV file containing trades"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--holdings", action="store_true", help="Display current holdings"
        )
        args = parser.parse_args()

        if not args.dry_run:
            require_env_vars(["CONSUMER_KEY", "MOBILE_NO", "UCC", "MPIN"])
        ...
        if args.holdings:
            ...
            for stock in stocks:
                tracker_id = stock["stock_id"] + datetime.datetime.now().strftime(
                    "%Y%m%d%H%M%S"
                )
                trade_details = {
                ...
                if book_trade(client, trade_details):
                    success_count += 1
                else:
                    fail_count += 1
